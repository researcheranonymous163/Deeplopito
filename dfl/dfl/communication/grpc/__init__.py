...
class GrpcCommunicator(Communicator, GrpcCommunicatorServicer):
	...
	async def _release_client(self, name: str, grace: timedelta | None = None) -> None:
		name = GrpcCommunicator._normalize_name(name)
		
		_, channel = self._clients[name]
		await channel.close(grace=grace.total_seconds() if grace is not None else sys.float_info.max)
		del self._clients[name]
	
	async def close(self, grace: timedelta | None = None) -> None:
		await super().close()
			...
			async with self._clients_lock:
				...
				async with asyncio.TaskGroup() as tg:
					for name in self._clients.keys():
						tg.create_task(shield_exception(self._release_client(name, grace=grace)))
		
		async with asyncio.TaskGroup() as tg:
			tg.create_task(shield_exception(_release_all_clients()))
			tg.create_task(shield_exception(self._server.stop(grace=grace.total_seconds() if grace is not None else sys.float_info.max)))
